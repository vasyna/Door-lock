...
#Логика работы
- Замок замкнется только при условии закрытой двери и замкнутого состояния геркона выдвинутого ригеля
- Замок отроется при условии замкнутого состояния геркона задвинутого ригеля
- В случае если при любой команде состояние ригеля не определено, прозвучит двойной сигнал и он будет открыт.
...
Опорное напряжение на драйвере выставил в 0.7В, что составляет примерно ток в 1А. При работе ни драйвер, ни мотор не грелись.
